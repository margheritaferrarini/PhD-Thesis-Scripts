...
def binomial_test(ntimes_freq, test):	#test_type should be greater [G] if we want one-tailed test or two-sided [T] for two-tailed test 
	...
	pvalue = stats.binom_test(ntimes, n=sample_number, p=float(freq), alternative=test_type)
...
if __name__=='__main__':
	...
	#parser.add_argument("-n", "--samples-number", metavar="NUM", type=int, required=True, help="Number of samples included into 1000G study. It depends on the phase of the project.")
	...
	try:
		#embed()
		final_df = pandas.DataFrame()
		with open(args.output_file + '.pickle', 'rb') as final_df_file:
			try:
				all_finders = pickle.load(final_df_file)
			except:
				all_finders = pandas.read_pickle(final_df_file)
			final_df = all_finders['final_df']
		#embed()
		final_df_file.close()
		...
		if args.input_file.endswith('.gz'):
			VCF = gzip.open(args.input_file, 'r')
		else:
			VCF = open(args.input_file, 'r')
		# VCF reading and computing allele count
		for line in VCF:
			line = line.decode('utf-8')
			line_counter+=1
			if line_counter%10000 == 0:
				print('{} vcf lines processed'.format(line_counter), file=sys.stderr)
			...
			if not line.startswith('#'):
				...
				for i, alt in enumerate(ALTS):
					...
					for sample in columns[9:]:
						...
						#	embed()
						if sample.startswith('./.') or sample.startswith('0|0') or sample.startswith('0/0') or (sample == '0'):	# problem with X,Y and MT
							homo_ref_dict[key] += 1 
							#continue
						elif sample.startswith('{}/{}'.format(i+1,i+1)) or sample.startswith('{}|{}'.format(i+1,i+1)) or ([sample == str(i) for i in range(50)].count(True) > 0): 
							allele_count_dict[key] += 2
							homo_alt_dict[key] += 1 
						elif sample.startswith('{}/'.format(i+1)) or sample.startswith('{}|'.format(i+1)): 		# es 1/2, 1/3, 1/0
							allele_count_dict[key] += 1				
							het_dict[key] += 1 
						elif sample[2] == str(i+1):						# es 0/1, 3/1, 0|1, 3|1
							allele_count_dict[key] += 1
							het_dict[key] += 1 
					#	old_key = key
					variant_number += 1 
		...
		print('{} vcf lines processed in {}s.'.format(line_counter, final_reading_time), file=sys.stderr)
...
#
		...
		final_df.loc[mt_chrom, 'chrom'] = 'MT'
		...
		try:
			final_df.to_pickle(args.output_file + '.pickle')
		except:
			with open(args.output_file + '.pickle', 'wb') as picklefile:
				pickle.dump(final_df, picklefile, pickle.HIGHEST_PROTOCOL)
